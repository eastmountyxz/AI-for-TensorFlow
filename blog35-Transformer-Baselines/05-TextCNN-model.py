...
cnn1 = Convolution1D(128, 3, padding='same', strides = 1, activation='relu')(layer)
cnn1 = MaxPool1D(pool_size=4)(cnn1)
cnn2 = Convolution1D(128, 4, padding='same', strides = 1, activation='relu')(layer)
cnn2 = MaxPool1D(pool_size=4)(cnn2)
cnn3 = Convolution1D(128, 5, padding='same', strides = 1, activation='relu')(layer)
...
if flag == "train":
    ...
    print("模型预测")
    model = load_model('Textcnn_model.h5') #导入已经训练的模型
    test_pre = model.predict(test_seq_mat)    
    confm = metrics.confusion_matrix(np.argmax(test_pre, axis=1),
                                     np.argmax(test_y, axis=1))
    print(confm)
    with open("TextCNN-pre-result.txt","w") as f: #结果保存
    ...
    plt.savefig('TextCNN-result.png')
    plt.show()